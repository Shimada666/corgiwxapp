...
<template>
  <tm-app ref="app">
    <tm-sticky>
      <template #sticky>
        <tm-sheet
          :text="false" :linear="store.tmStore.dark ? '' : 'bottom'" :linear-color="headerBgColor"
          :margin="[0, 0]" :follow-theme="true"
        >
          ...
          <view class="flex-row flex-row-center-start pb-10">
            ...
            <view class="pl-16 flex-1" style="width: 0px">
              <tm-text
                :folow-theme="false" _class="text-weight-b" :font-size="36"
                :label="tmuiVer"
              />
              <tm-text
                :folow-theme="false" _class="opacity-6"
          ...
          <view class="px-0 pt-12">
            <tm-input
              v-model="str" focus-color="white" :text="false" color="white"
              :round="16" :transprent="store.tmStore.dark" :placeholder="language('index.search.tips')"
              show-clear prefix="tmicon-search" @confirm="search"
            />
          </view>
        </tm-sheet>
      </template>
      <tm-sheet :shadow="6" :margin="[24]" :padding="[24]" :round="6">
        <view class="flex flex-row flex-around">
          <tm-button
            :round="8" :width="100" color="yellow" size="small" :label="language('index.com.themeGreen')"
            @click="setTheme('yellow')"
          />
          <tm-button
            :round="8" :width="100" color="blue" size="small" :label="language('index.com.themeBlue')"
            @click="setTheme('blue')"
          />
          <tm-button
            :round="8" :width="100" color="red" size="small" :label="language('index.com.themeRed')"
            @click="setTheme('red')"
          />
          <tm-button
            :round="8" color="brown" :width="100" size="small" :label="language('index.com.themeCustText')"
            @click="showCustom = true"
          />
          <tm-button
            :round="8" :width="160" size="small" :label="language('index.com.themeDefault')"
            @click="setTheme('')"
          />
        </view>
      </tm-sheet>
      ...
      <view class="mx-24 ">
        <tm-row :width="702" :column="3" :round="6" :transprent="false">
          <tm-col
            v-for="(item, index) in category" :key="index" :transprent="true"
            :height="190" @click="toLinks(item.url)"
          >
            <tm-icon :color="item.color" _class="pb-10" :font-size="52" :name="item.icon" />
            <tm-text :font-size="28" _class="text-weight-b" :label="language(item.title)" />
            <tm-text color="grey" :font-size="22" :label="language(item.label)" />
          </tm-col>
        </tm-row>
      </view>
      ...
      <view class="py-24 ">
        <!-- #ifdef MP-WEIXIN -->
        <tm-cell
          url="../wxaccount/user" :margin="[24, 0, 24, 16]" show-avatar :round="4" :title-font-size="30"
          :title="language('index.com.love')" label="可登录模板市场" :right-text="language('index.com.loveSub')"
        >
          <template #avatar>
            <tm-icon color="orange" :font-size="38" name="tmicon-heart-fill" />
          </template>
        </tm-cell>
        <!-- #endif -->
        <tm-cell
          :margin="[24, 0, 24, 16]" show-avatar :round="4" :title-font-size="30" :title="language('index.com.setLocal')"
          :right-text="language('language')" @click="seLocal"
        >
          <template #avatar>
            <tm-icon color="primary" :font-size="38" name="tmicon-resource" />
          </template>
        </tm-cell>
        <tm-cell
          :margin="[24, 0]" show-avatar :round="4" :title-font-size="30"
          :title="language('index.com.autoDark')"
        >
          <template #avatar>
            <tm-icon color="pink" :font-size="38" name="tmicon-ios-color-palette" />
          </template>
          <template #right>
            <tm-checkbox v-model="autoDark" :round="24" @change="autoDarkChange" />
          </template>
        </tm-cell>
      </view>
      ...
      <view class="py-32 flex flex-row flex-row-center-center">
        <view style="width: 300rpx">
          <tm-divider color="grey-2" :label="language('index.com.bottom')" />
        </view>
      </view>
    </tm-sticky>

    <view style="height: 100px" />
    <tm-float-button :btn="{ icon: 'tmicon-ios-sunny' }" @click="onChangeDark" />
    <tm-message ref="msg" />
    <tm-drawer v-model:show="showCustom" placement="center" hide-header :height="450" :width="600">
      <view class="pa-32 flex flex-col">
        <view class="text-align-center py-24">
          <tm-text _class="text-weight-b" :font-size="32" label="自定义主题" />
        </view>
        <tm-input
          v-model="showCustomColor" :round="16" prefix-label="颜色值" placeholder="请输入颜色值,比如:#FF00FF" :border="1"
          show-clear
        />
        <tm-input
          v-model="showCustomName" :round="16" prefix-label="颜色名称" :margin="[0, 24]" placeholder="字母,如:darkGreen"
          :border="1" show-clear
        />
        <tm-button :round="16" block label="确认切换" @click="changeCustomColor" />
      </view>
    </tm-drawer>
  </tm-app>
</template>
